...
SELECT department_name, round(avg(salary)) from employees JOIN departments USING(department_id) GROUP BY department_name
ORDER BY round(avg(salary)) DESC LIMIT 1;
...
HAVING COUNT(*) > 2;
SELECT customer_id, count(*) as ordered, sum(total_amount) from orders GROUP BY customer_id HAVING count(*) > 2;

SELECT extract(MONTH from order_date) as order_month, sum(total_amount)  
FROM orders 
